...
- **Contact profiles**: A full profile for every contact with notes, attached files, upcoming events, activity timeline, and email history. Everything in one place before you hit send.
...
**Local-first, always.** No server means no recurring cost, no data leaving your machine, and no dependency on my uptime. The tradeoff is that there's no sync across devices in v0.2. Worth it.
 
**Deferred the AI layer.** I had planned AI-assisted email drafting for v0.2. I cut it deliberately to ship faster. A product that ships without AI beats a product that never ships with it.
